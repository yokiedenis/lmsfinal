...
//     // Fetch the chapterId from the ChapterQuiz
...
//     if (!chapterQuiz) {
//       return NextResponse.json(
//         { message: "ChapterQuiz not found" },
//         { status: 404 }
//       );
//     }

//     const chapterId = chapterQuiz.chapterId;
...
//           score,
//           totalQuestions,
//           chapterId, // Include the required chapterId field
//           answers: JSON.stringify(answers),
//         },
//       });
//     }
...
export async function POST(
  request: NextRequest,
  { params }: { params: { courseId: string; chapterId: string } } // Updated to chapterId
) {
  const { chapterId } = params; // Note: Renamed from chapterQuizId to match URL param
  ...
  try {
    ...
    const chapterQuiz = await prisma.chapterQuiz.findUnique({
      where: { chapterId }, // Assuming chapterId is unique in ChapterQuiz
      select: { id: true },
    });

    if (!chapterQuiz) {
      return NextResponse.json(
        { message: "ChapterQuiz not found for this chapter" },
        { status: 404 }
      );
    }
    ...
    // Iterate through answers to calculate the score
    answers.forEach((answer) => {
      const submittedAnswer = answer.answer.split(".")[0].trim(); // Extract the letter and trim whitespace
      const correctAnswer = correctAnswersMap[answer.questionId];
    ...
    if (existingAttempt) {
      ...
      quizAttempt = await prisma.chapterQuizAttempt.create({
        data: {
          ...
          score,
          totalQuestions,
          chapterId, // Include the required chapterId from params
          answers: JSON.stringify(answers),
        },
      });
    }
