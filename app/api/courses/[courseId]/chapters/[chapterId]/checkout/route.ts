...
//         userId: user.id,
//         courseId: params.courseId,
//         chapterId: params.chapterId, // Added missing chapterId
//         amount: amount,
//         dpoToken: token,
//         status: "PENDING",
//       },
...
const verifyToken = async (token: string) => {
  ...
  try {
    ...
    if (parsedResponse.API3G.Result === "000") {
      return {
        success: true,
        status: "COMPLETED", // Explicitly setting the status we want
      ...
      return {
        success: false,
        status: "FAILED",
        error: parsedResponse.API3G.ResultExplanation
      };
    }
  } catch (error) {
    if (axios.isAxiosError(error)) {
...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (!verificationResult.success) {
      return new NextResponse(`Payment verification failed: ${verificationResult.error}`, { status: 400 });
    }
    ...
    const updatedTransaction = await db.transaction.update({
      ...
      data: {
        status: verificationResult.status, // Use the status from verification
      },
    });
    ...
    const purchase = await db.purchase.create({
      ...
      // Rollback transaction status if purchase creation fails
      await db.transaction.update({
