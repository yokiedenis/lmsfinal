...
//     try {
//       const token = await getToken();
//       console.log('Clerk Token (Reply):', token ? 'Present' : 'Missing');
//       if (!token) throw new Error('No authentication token found');
...
//       setPosts(prevPosts =>
//         prevPosts.map(p =>
//           p.id === postId ? updatedPostResponse : p
//         )
//       );
//       addNotification(`${currentUser.name} replied to "${post.title}"`, 'new-reply', postId);
//       setReplyContent(prev => ({ ...prev, [postId]: '' }));
...
//     try {
//       const token = await getToken();
//       console.log('Clerk Token (Pin):', token ? 'Present' : 'Missing');
//       if (!token) throw new Error('No authentication token found');
...
//       const updatedPostResponse = await response.json();
//       setPosts(prevPosts =>
//         prevPosts.map(p => (p.id === postId ? updatedPostResponse : p))
//       );
//       if (!post.isPinned) {
//         addNotification(`Post "${post.title}" was pinned by instructor`, 'update', postId);
//       }
...
//     try {
//       const token = await getToken();
//       console.log('Clerk Token (Delete):', token ? 'Present' : 'Missing');
//       if (!token) throw new Error('No authentication token found');
...
export default function Forum() {
  ...
  const handleCreatePost = async () => {
    ...
    const timestamp = Date.now();
    const { date, timeSince } = formatPostDate(timestamp);
    const postId = `${timestamp}-${Math.random().toString(36).substr(2, 9)}`; // Generate unique ID
    ...
    try {
      ...
      const createdPost = await response.json(); // Read response.json() only once
      console.log('Create Post Response:', response.status, createdPost);
  ...
  const togglePinPost = async (postId: string) => {
    ...
    try {
      ...
      console.error('Error toggling pin:', error);
      setErrorMessage(error instanceof Error ? error.message : 'Failed to pin post.');
    }
  };
