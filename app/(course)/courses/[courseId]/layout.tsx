...
const CourseLayout = async ({
  ...
  const { userId } = auth();
  if (!userId) {
    console.log("No userId found, redirecting to /");
    return redirect("/");
  }
  ...
  if (!params.courseId || params.courseId.includes(":courseId")) {
    console.error("Invalid courseId detected:", params.courseId);
    return redirect("/");
  }

  if (typeof params.courseId !== "string" || params.courseId.trim() === "") {
    console.error("Invalid courseId (empty or non-string):", params.courseId);
    return redirect("/");
  }

  console.log("Fetching course with ID:", params.courseId); // Debug log
  ...
  console.log("Course found:", course.id); // Debug log

  const progressCount: number | null = await getProgress(userId, course.id);
  const quizId: string | null = course.quizzes?.[0]?.id ?? null;
